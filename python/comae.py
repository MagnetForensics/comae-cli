...
import requests, time, subprocess, argparse, math, sys, os
import cloud_upload, util, comae_api
...
def handle_file(file, args, filetype):
    ...
    if args.action == "upload-comae":
        ...
        api_key = comae_api.getApiKey(args.client_id, args.client_secret)
        print("[COMAE] Uploading file to Comae Stardust")

        if filetype == "dump":
            comae_api.sendDumpToComae(file, api_key)

        if filetype == "snap":
            comae_api.sendSnapshotToComae(file, api_key)
    ...
    if args.action == "upload-gcp":
        ...
        if not "gcp_bucket" in args:
            print("Please provice a bucket name with --gcp-bucket")
            fail = True
        ...
        cloud_upload.upload_gcp(args.gcp_bucket, file)
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--action", help='One of "store", "upload_comae", "upload_gcp", "upload_az", "upload_s3"', default="store")
    ...
    argparser.add_argument("--gcp-bucket", help="Name of bucket to use if uploading to GCP")
    ...
    if args.get_api_key:
        print(comae_api.getApiKey(args.client_id, args.client_secret))
